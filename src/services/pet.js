...
const _getOne = async (id) => {
    try {
        const result = await session.run(`MATCH (p:Pet) WHERE ID(p) = ${id} RETURN p`);
...
const _deleteOne = async (id) => {
    try {
        const result = await session.run(`MATCH (p:Pet) WHERE ID(p) = ${id} DELETE p`);
